#!/usr/bin/env python3
"""
Step 5: Longform Evaluation - Transcribe Response Audio
Uses Whisper ASR to transcribe the task response audio.
"""
...
class LongformTranscriber:
    def __init__(self):
        self.transcript_output_dir = Path("longform_transcripts")
    ...
    def transcribe_audio(self, audio_path: str, output_filename: str) -> str:
        ...
        print(f"\n{'='*60}")
        print(f"LONGFORM EVALUATION - TRANSCRIPTION")
        print(f"{'='*60}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Step 5: Transcribe longform task response audio using Whisper ASR"
    )
    parser.add_argument("input_audio", help="Path to audio WAV file to transcribe")
    parser.add_argument("output_filename", help="Output transcript filename (e.g., therapy_001_transcript.txt)")
